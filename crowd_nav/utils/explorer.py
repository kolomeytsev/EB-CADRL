...
class Explorer(object):
    ...
    def run_one_episode(self, phase, update_memory, imitation_learning, episode_number):
        ...
        if isinstance(info, ReachGoal):
            ...
            self.collision_human += 1
            self.collision_cases_human.append(episode_number)
            self.collision_times_human.append(self.env.global_time)
    ...
    def compile_metrics(self) -> Dict:
        return {
            ...
            "collision_rate_human": self.collision_rate_human,
            ...
            "Collision Human cases:": [str(x) for x in self.collision_cases_human],
    ...
    def log_results(self, print_failure: bool = False, num_episodes: int = None, phase: str = None,
        ...
        self.collision_rate_human = self.collision_human / num_episodes
        ...
        assert (self.success + self.collision + self.collision_human + self.collision_bicycle + \
        ...
        logging.info('{:<5} {}self.collision rate human: {:.2f}, self.collision rate bicycle: {:.2f},'
                     'self.collision rate child: {:.2f}, self.collision rate obstacle: {:.4f}'.
            format(phase.upper(), extra_info, self.collision_rate_human, self.collision_rate_bicycle,
                   self.collision_rate_child, self.collision_rate_obstacle))

        self.num_step = None
        if phase in self.PHASES:
            self.num_step = sum(
                self.success_times + self.collision_times + self.collision_times_human + \
        ...
        if print_failure:
            ...
            logging.info('Collision Human cases: ' + ' '.join([str(x) for x in self.collision_cases_human]))
