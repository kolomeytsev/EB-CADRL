...
from crowd_nav.utils.utils import configure_policy, configure_environment_and_robot

from simulator.agents.robot import Robot
import multiprocessing
# from multiprocessing import Pool, cpu_count
from simulator.utils.info import (ReachGoal, Collision, CollisionChild, CollisionHuman,
...
def calculate_cumulative_reward(rewards, gamma, time_step, v_pref) -> float:
    return sum([
        pow(gamma, t * time_step * v_pref) * reward for t, reward in enumerate(rewards)])


def run_one_episode(args, phase, policy, imitation_learning, episode):
    try:
        torch.multiprocessing.set_sharing_strategy('file_system')
        robot, env = configure_environment_and_robot(args, 'CrowdSimStatic-v0')
        ...
        collision_flag = isinstance(info, Collision) or isinstance(info, CollisionHuman) or \
                isinstance(info, CollisionBicycle) or isinstance(info, CollisionObstacle) or \
                isinstance(info, CollisionChild)

        success = 0
        collision = collision_child = collision_human = collision_bicycle = collision_obstacle = 0
        timeout = 0
        ...
        collision_rate_human = collision_human
        collision_rate_bicycle = collision_bicycle
        collision_rate_child = collision_child
        collision_rate_obstacle = collision_obstacle

        assert (success + collision + collision_human + collision_bicycle + \
                collision_obstacle + collision_child + timeout) == 1

        avg_nav_time = env.global_time

        extra_info = '' if episode is None else 'in episode {} '.format(episode)

        cum_reward = calculate_cumulative_reward(rewards, policy.gamma, env.time_step, robot.v_pref)
        logging.info('{:<5} {} 1 has success rate: {:.2f}, collision rate: {:.2f}, nav time: {:.2f}, total reward: {:.4f}'.
            format(phase.upper(), extra_info, success_rate, collision_rate, avg_nav_time, cum_reward))

        logging.info('{:<5} {}collision rate human: {:.2f}, collision rate bicycle: {:.2f},'
                        'collision rate child: {:.2f}, collision rate obstacle: {:.4f}'.
            format(phase.upper(), extra_info, collision_rate_human, collision_rate_bicycle,
                    collision_rate_child, collision_rate_obstacle))
...
class ParallelExplorer(object):
    ...
    def __init__(self, env, robot, device, processes_num, memory=None, gamma=None, target_policy=None):
    ...
    def process_results(self, results, update_memory, imitation_learning):
        ...
        for collision_flag, states, actions, rewards, info, avg_nav_time, cum_reward in results:
            if update_memory:
                # collision_flag = isinstance(info, Collision) or isinstance(info, CollisionHuman) or \
                #     isinstance(info, CollisionBicycle) or isinstance(info, CollisionObstacle) or \
                #     isinstance(info, CollisionChild)
    ...
    def log_val_aggregated(self, results):
        ...
        collision_rate_human = collision_human / episodes_num
        ...
        logging.info('VAL_AGGREGATED has success rate: {:.2f}, collision rate: {:.2f}, nav time: {:.2f}, total reward: {:.4f}'.
            format(success_rate, collision_rate, nav_time_avg, cum_reward_avg))

        logging.info('VAL_AGGREGATED collision rate human: {:.2f}, collision rate bicycle: {:.2f},'
                        'collision rate child: {:.2f}, collision rate obstacle: {:.4f}'.
            format(collision_rate_human, collision_rate_bicycle,
                   collision_rate_child, collision_rate_obstacle))

    def run_k_episodes_parallel(self, args, start_episode, end_episode, phase,
                                update_memory=False, imitation_learning=False):
        torch.multiprocessing.set_sharing_strategy('file_system')
    ...
    def update_memory(self, states, actions, rewards, imitation_learning=False):
        ...
        for i, state in enumerate(states):
            ...
            if imitation_learning:
                ...
                value = sum([pow(self.gamma, max(t - i, 0) * self.time_step * self.v_pref) * reward
    ...
    def calculate_cumulative_reward(self, rewards) -> float:
        return sum([
            pow(self.gamma, t * self.time_step * self.v_pref) * reward for t, reward in enumerate(rewards)])
