...
class CADRL(Policy):
    ...
    def propagate(self, state, action):
        if isinstance(state, ObservableState):
            ...
            next_state = ObservableState(
                next_px, next_py, action.vx, action.vy, state.radius, state.obj_type
            )
    ...
    def rotate(self, state):
        ...
        if self.with_agent_type:
            agent_type_ohe = one_hot(
                state[:, 14].long(), num_classes=self.agent_type_state_dim
            )
            new_state = torch.cat(
                [
                    dg,
