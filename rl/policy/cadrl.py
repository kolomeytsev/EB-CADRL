...
class CADRL(Policy):
    ...
    def propagate(self, state, action):
        if isinstance(state, ObservableState):
            # propagate state of adults
            next_px = state.px + action.vx * self.time_step
            next_py = state.py + action.vy * self.time_step
            next_state = ObservableState(next_px, next_py, action.vx, action.vy, state.radius, state.obj_type)
            ...
            if self.kinematics == 'holonomic':
                ...
                next_state = FullState(next_px, next_py, action.vx, action.vy, state.radius,
                                       state.gx, state.gy, state.v_pref, state.theta, state.obj_type)
            else:
                ...
                next_state = FullState(next_px, next_py, next_vx, next_vy, state.radius, state.gx, state.gy,
                                       state.v_pref, next_theta, state.obj_type)
        else:
    ...
    def transform(self, state):
        ...
        state = torch.Tensor(state.self_state + state.agent_states[0]).to(self.device)
    ...
    def rotate(self, state):
        ...
        """
        # 'px', 'py', 'vx', 'vy', 'radius', 'gx', 'gy', 'v_pref', 'theta', 'px1', 'py1', 'vx1', 'vy1', 'radius1' 'agent_type'
        ...
        if self.with_agent_type:
            agent_type_ohe = one_hot(state[:, 14].long(), num_classes=self.agent_type_state_dim)
            new_state = torch.cat(
                [dg, v_pref, theta, radius, vx, vy, px1, py1, vx1, vy1, radius1, da, radius_sum, agent_type_ohe],
                dim=1)
        else:
            new_state = torch.cat(
