...
class Explorer(object):
    ...
    def run_k_episodes(
        ...
        for episode_number in range(num_episodes):
            self.run_one_episode(
                phase, update_memory, imitation_learning, episode_number
            )
    ...
    def compile_metrics(self) -> Dict:
        return {
            ...
            "Frequency of being in danger": self.too_close / self.num_step
            if self.num_step
            else None,
            ...
            "Collision Obstacle cases:": [
                str(x) for x in self.collision_cases_obstacle
            ],
    ...
    def log_results(
        ...
        assert (
            self.success
            + self.collision
            + self.collision_adult
            + self.collision_bicycle
        ...
        self.avg_nav_time = (
            sum(self.success_times) / len(self.success_times)
        ...
        logging.info(
            "{:<5} {}has success rate: {:.2f}, self.collision rate: {:.2f}, nav time: {:.2f}, total reward: {:.4f}".format(
                phase.upper(),
                extra_info,
                self.success_rate,
                self.collision_rate,
        ...
        logging.info(
            "{:<5} {}self.collision rate adult: {:.2f}, self.collision rate bicycle: {:.2f},"
            "self.collision rate child: {:.2f}, self.collision rate obstacle: {:.4f}".format(
                phase.upper(),
                extra_info,
                self.collision_rate_adult,
                self.collision_rate_bicycle,
                self.collision_rate_child,
                self.collision_rate_obstacle,
            )
        )
        ...
        if print_failure:
            logging.info(
                "Collision cases: " + " ".join([str(x) for x in self.collision_cases])
            )
            logging.info(
                "Collision Adult cases: "
                + " ".join([str(x) for x in self.collision_cases_adult])
            )
            logging.info(
                "Collision Bicycle cases: "
                + " ".join([str(x) for x in self.collision_cases_bicycle])
            )
            logging.info(
                "Collision Child cases: "
                + " ".join([str(x) for x in self.collision_cases_child])
            )
            logging.info(
                "Collision Obstacle cases: "
                + " ".join([str(x) for x in self.collision_cases_obstacle])
            )
            logging.info(
                "Timeout cases: " + " ".join([str(x) for x in self.timeout_cases])
            )
