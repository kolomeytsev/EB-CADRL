...
def parse_arguments():
    ...
    parser.add_argument(
        "--env_config", type=str, default="configs/env_configs/env.config"
    )
    parser.add_argument("--policy", type=str, default="cadrl")
    parser.add_argument(
        "--policy_config", type=str, default="configs/policy_configs/policy.config"
    )
    parser.add_argument(
        "--train_config", type=str, default="configs/train_configs/train.config"
    )
...
def configure_paths(args):
    ...
    if os.path.exists(args.output_dir):
        if args.resume and args.resume_iteration > 0:
            make_new_dir = False
            args.env_config = os.path.join(
                args.output_dir, os.path.basename(args.env_config)
            )
...
def run_imitation_learning(
    trainer, explorer, robot, policy, args, memory, model, train_config
):
    il_weight_file = os.path.join(args.output_dir, "il_model.pth")

    if args.resume and args.resume_iteration != 0:
        rl_weight_file = os.path.join(
            args.output_dir, f"rl_model_{args.resume_iteration}.pth"
        )
        ...
        il_learning_rate = train_config.getfloat(
        ...
        logging.info(
            "Finish imitation learning. Weights saved. Time passed: %f",
            time.time() - t0,
        )
...
def run_train(args, policy, env, robot):
    ...
    explorer = Explorer(
        env, robot, policy.device, memory, policy.gamma, target_policy=policy
    )
    ...
    while episode < train_episodes:
        ...
        if episode < epsilon_decay:
            epsilon = (
                epsilon_start + (epsilon_end - epsilon_start) / epsilon_decay * episode
            )
        else:
            epsilon = epsilon_end
        ...
        if episode % evaluation_interval == 0:
            try:
                parallel_explorer.run_k_episodes_parallel(
                    args,
                    VAL_EPISODE_START,
                    VAL_EPISODE_START + env_config.getint("env", "val_size"),
                    "val",
                )
            except RuntimeError as e:
                logging.info(
                    "Caught RuntimeError in run_k_episodes_parallel val: {}".format(
                        str(e)
                    )
                )
        ...
        try:
            ...
            logging.info(
                "Caught RuntimeError in run_k_episodes_parallel train: {}".format(
                    str(e)
                )
            )
        ...
        if episode % target_update_interval == 0:
            try:
                ...
                logging.info(
                    "Caught RuntimeError in update_target_model: {}".format(str(e))
                )
        ...
        if args.end_iteration > 0 and episode >= args.end_iteration:
            logging.info(
                "Ending train on episode: {episode}, end_iteration: {args.end_iteration}"
            )
            break
...
def main():
    ...
    t0 = time.time()
    logging.info(
        f"Run train with arguments: {' '.join(f'{k}={v}' for k, v in vars(args).items())}"
    )
