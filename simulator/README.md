...
## Environment
The environment contains n+1 agents. N of them are adults controlled by certain unknown
...
## Agent
Agent is a base class, and has two derived class of adult and robot. Agent class holds
all the physical properties of an agent, including position, velocity, orientation, policy and etc.
* visibility: adults are always visible, but robot can be set to be visible or invisible
...
* CADRL: learn a value network to predict the value of a state and during inference it predicts action for the most important adult
* LSTM-RL: use lstm to encode the adult states into one fixed-length vector
* SARL: use pairwise interaction module to model adult-robot interaction and use self-attention to aggregate adults' information
* OM-SARL: extend SARL by encoding intra-adult interaction with a local map
