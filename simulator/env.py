...
class EntityBasedCollisionAvoidance(gym.Env):
    ...
    def reset_times(self, phase):
        if phase == 'test':
            ...
            self.adult_times = [0] * (self.scene.adult_num if self.robot.policy.multiagent_training else 1)
    ...
    def reset(self, phase='test', test_case=None, imitation_learning=False, compute_local_map=True,
        ...
        for agent in [self.robot] + self.scene.adults + self.scene.bicycles + self.scene.children:
        ...
        ob = [
            agent.get_observable_state() for agent in self.scene.adults + self.scene.bicycles + self.scene.children
        ]
    ...
    def compute_collisions(self, all_agents, action, border):
        # collision detection
        collision_adult, dmin_adult = self.compute_collision_with_agents(self.scene.adults, action)
        ...
        return dmin_adult, dmin_bicycle, dmin_child, collision_adult, \
    ...
    def step(self, action, update=True, compute_local_map=True, border=None):
        ...
        dmin_adult, dmin_bicycle, dmin_child, collision_adult, collision_bicycle, collision_obstacle, collision_child = self.compute_collisions(
            all_agents, action, border)

        reward, done, info = self.reward.compute(
            dmin_adult, dmin_bicycle, dmin_child, collision_adult, collision_bicycle,
    ...
    def render(self, mode, output_file=None, deconv=None, plot_agents_goals_flag=True, frame=0):
        ...
        '''
        if mode == 'traj':
            render_trajectory(self.states, self.scene.adults, self.scene.bicycles, self.scene.children,
            ...
            render_video(
                self.states, self.last_circle_radius, self.robot.radius, self.scene.adults, self.scene.bicycles,
                self.scene.children, self.scene.static_obstacles_as_pedestrians, self.local_maps, self.use_grid_map,
                self.scene.obstacle_vertices, self.robot.kinematics, self.scene.adult_num, self.submap_size_m,
