...
from simulator.utils.render import (
    render_trajectory, render_am, render_traj_3D, render_og, render_video
)
from simulator.scene.scene_generator import SceneGenerator


class CrowdSimStatic(gym.Env):
    ...
    def __init__(self):
        ...
        Agent can either be human, bicycle or robot.
        humans and bicycles are controlled by a unknown and fixed policy.
        robot is controlled by a known and learnable policy.

        """
        self.name = 'CrowdSimStatic'
    ...
    def reset_times(self, phase):
        if phase == 'test':
            self.human_times = [0] * self.scene.human_num
        else:
            self.human_times = [0] * (self.scene.human_num if self.robot.policy.multiagent_training else 1)

        if phase == 'test':
            self.bicycle_times = [0] * self.scene.bicycle_num
        else:
            self.bicycle_times = [0] * (self.scene.bicycle_num if self.robot.policy.multiagent_training else 1)

        if phase == 'test':
            self.children_times = [0] * self.scene.children_num
        else:
            self.children_times = [0] * (self.scene.children_num if self.robot.policy.multiagent_training else 1)

    def reset(self, phase='test', test_case=None, imitation_learning=False, compute_local_map=True,
              save_scene_path=None, load_scene_path=None, scene_number=None):
        """
        ...
        self.robot.set(0, -self.scene.circle_radius, 0, self.scene.circle_radius, 0, 0, np.pi / 2)
        ...
        if load_scene_path is not None:
            ...
            self.scene.generate_random_scene(counter_offset, phase, save_scene_path, scene_number=scene_number)

        for agent in [self.robot] + self.scene.humans + self.scene.bicycles + self.scene.children:
        ...
        ob = [
            agent.get_observable_state() for agent in self.scene.humans + self.scene.bicycles + self.scene.children
        ]
    ...
    def collision_detection_between_agents(self, all_agents):
        human_num = len(all_agents)
        # Optimize
        for i in range(human_num):
            for j in range(i + 1, human_num):
                dx = all_agents[i].px - all_agents[j].px
                dy = all_agents[i].py - all_agents[j].py
                dist = (dx ** 2 + dy ** 2) ** (1 / 2) - all_agents[i].radius - all_agents[j].radius
                if dist < 0:
    ...
    def compute_collision_with_obstacle(self, action, border):
        ...
        end_idx_x = min(end_idx_x, int(round(self.scene.map_size_m / self.scene.map_resolution)))

        start_idx_y = robot_idx_map_y - robot_size_map
        end_idx_y = start_idx_y + robot_size_map * 2

        start_idx_y = max(start_idx_y, 0)
        end_idx_y = min(end_idx_y, int(round(self.scene.map_size_m / self.scene.map_resolution)))

        if end_idx_x > start_idx_x and end_idx_y > start_idx_y:
            map_around_robot = self.scene.map[start_idx_x:end_idx_x,
        ...
        robot_size_map = int(
            np.ceil((self.robot.radius + self.reward.discomfort_dist) / self.scene.map_resolution))

        start_idx_x = robot_idx_map_x - robot_size_map
        end_idx_x = start_idx_x + robot_size_map * 2
        start_idx_x = max(start_idx_x, 0)
        end_idx_x = min(end_idx_x,
                        int(round(self.scene.map_size_m / self.scene.map_resolution)))
        start_idx_y = robot_idx_map_y - robot_size_map
        end_idx_y = start_idx_y + robot_size_map * 2
        start_idx_y = max(start_idx_y, 0)
        end_idx_y = min(end_idx_y,
                        int(round(self.scene.map_size_m / self.scene.map_resolution)))
        if end_idx_x > start_idx_x and end_idx_y > start_idx_y:
            larger_map_around_robot = self.scene.map[start_idx_x:end_idx_x, start_idx_y:end_idx_y]
    ...
    def compute_collisions(self, all_agents, action, border):
        # collision detection
        collision_human, dmin_human = self.compute_collision_with_agents(self.scene.humans, action)
        collision_bicycle, dmin_bicycle = self.compute_collision_with_agents(self.scene.bicycles, action)
        collision_child, dmin_child = self.compute_collision_with_agents(self.scene.children, action)
        ...
        return dmin_human, dmin_bicycle, dmin_child, collision_human, \
                collision_bicycle, collision_obstacle, collision_child
    ...
    def compute_step_update(self, action, agents_actions, all_agents):
        ...
        self.states.append([
            ...
            [human.get_full_state() for human in self.scene.humans],
        ...
        for i, human_action in enumerate(agents_actions):
            all_agents[i].step(human_action)

        self.global_time += self.time_step

        for i, human in enumerate(self.scene.humans):
    ...
    def step(self, action, update=True, compute_local_map=True, border=None):
        ...
        """
        agents_actions = []
        all_agents = self.scene.humans + self.scene.bicycles + self.scene.children
        for agent_i, agent in enumerate(all_agents):
            # observation for humans and bicycles is always coordinates
            ob = [other_agent.get_observable_state() for other_agent in all_agents if other_agent != agent]
            if self.robot.visible:
                ob += [self.robot.get_observable_state()]

            agents_action = agent.act(ob)  # human.act(ob, self.scene.obstacle_vertices)
            agents_actions.append(agents_action)

            # TODO uncomment:
            """
            if self.human_policy == 'orca':
                agents_actions.append(human.act(ob, self.scene.obstacle_vertices))
                ...
                agents_actions.append(human.act())
            else:
                agents_actions.append(human.act(ob))
            """

        dmin_human, dmin_bicycle, dmin_child, collision_human, collision_bicycle, collision_obstacle, collision_child = self.compute_collisions(
            all_agents, action, border)

        reward, done, info = self.reward.compute(
            dmin_human, dmin_bicycle, dmin_child, collision_human, collision_bicycle,
            collision_obstacle, collision_child, action, self.global_time)

        if update:
            ob = self.compute_step_update(action, agents_actions, all_agents)
        else:
            if self.robot.sensor == 'coordinates':
                ob = [agent.get_next_observable_state(action) for agent, action in zip(all_agents, agents_actions)]
            elif self.robot.sensor == 'RGB':
                raise NotImplementedError
    ...
    def calculate_angular_map_distances(self, vertex, edge, theta, radial_dist_vector, rad_indeces, locations):
        ...
        for rad_idx_old, location in zip(rad_indeces, locations):
            ...
            for i in range(idx_diff):
                if (rad_idx < rad_idx_old and not wrapped) or (rad_idx > rad_idx_old and wrapped):
                    if (rad_idx + i) >= 0 and (rad_idx + i) < self.angular_map_dim:
                        ...
                        radial_dist_vector[(rad_idx + i) % self.angular_map_dim] = min(
                            radial_dist_vector[(rad_idx + i) % self.angular_map_dim], obstacle_value_in_slice)
                else:
                    if (rad_idx_old + i) >= 0 and (rad_idx_old + i) < self.angular_map_dim:
                        ...
                        obstacle_value_in_slice = np.linalg.norm([px, py])
                        radial_dist_vector[(rad_idx_old + i) % self.angular_map_dim] = min(
                            radial_dist_vector[(rad_idx_old + i) % self.angular_map_dim],
    ...
    def get_local_map(self, ob, append=True):
        ...
        """
        THRESHOLD_VALUE = 0.9
        center_idx_x = int(round((ob.px + self.scene.map_size_m / 2.0) / self.scene.map_resolution))
        center_idx_y = int(round((ob.py + self.scene.map_size_m / 2.0) / self.scene.map_resolution))
        ...
        if start_grid_y > end_grid_y or start_idx_y > end_idx_y \
            ...
            grid[start_grid_x:end_grid_x,
                 start_grid_y:end_grid_y] = self.scene.map[start_idx_x:end_idx_x,
                                                     start_idx_y:end_idx_y]
    ...
    def render(self, mode, output_file=None, deconv=None, plot_agents_goals_flag=True, frame=0):
        '''
        Visualizes the environment.
            @param mode: Choose 'video' for the rendering
        '''
        if mode == 'traj':
            render_trajectory(self.states, self.scene.humans, self.scene.bicycles, self.scene.children,
                self.robot.radius, self.time_step, self.scene.obstacle_vertices, self.last_circle_radius)
        elif mode == 'am':
            render_am(frame, self.states, self.scene.obstacle_vertices, self.local_maps_angular,
                self.angular_map_max_range, self.angular_map_dim, self.angular_map_max_angle,
                self.angular_map_min_angle, self.robot.radius)
        elif mode == 'traj3D':
            render_traj_3D(self.states, self.last_circle_radius, self.scene.humans)
        elif mode == 'og':
            render_og(frame, self.states, self.scene.obstacle_vertices, self.use_grid_map, self.local_maps,
                self.submap_size_m, self.scene.map_resolution, self.robot.radius)
        elif mode == 'video':
            render_video(
                self.states, self.last_circle_radius, self.robot.radius, self.scene.humans, self.scene.bicycles,
                self.scene.children, self.scene.static_obstacles_as_pedestrians, self.local_maps, self.use_grid_map,
                self.scene.obstacle_vertices, self.robot.kinematics, self.scene.human_num, self.submap_size_m,
                self.scene.map_resolution, self.time_step, self.angular_map_max_angle,
                self.angular_map_min_angle, self.angular_map_dim, self.angular_map_max_range,
                self.local_maps_angular, self.other_robots, self.attention_weights,
                output_file=output_file, deconv=deconv, plot_agents_goals_flag=plot_agents_goals_flag
            )
        else:
            raise NotImplementedError
