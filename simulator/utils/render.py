...
def render_trajectory(states, humans_to_render, bicycles_to_render, children_to_render, robot_radius, time_step,
    ...
    children_positions = [[state[2][j].position for j in range(
        len(children_to_render))] for state in states]
    ...
    for k in range(len(states)):
        if k % 4 == 0 or k == len(states) - 1:
            ...
            children = [plt.Circle(children_positions[k][i], children_to_render[i].radius, fill=False, color=CMAP(
                a)) for i, a in zip(range(len(children_to_render)), range(len(children_to_render)))]
    ...
    goal = lines.Line2D([0], [last_circle_radius], color=GOAL_COLOR, marker='*',
                        linestyle='None',
                        markersize=40,
                        label='Goal')
    ax.add_artist(goal)
...
def render_am(frame, states, obstacle_vertices, local_maps_angular, angular_map_max_range,
              angular_map_dim, angular_map_max_angle, angular_map_min_angle, robot_radius):
    ...
    angular_resolution = (angular_map_max_angle - angular_map_min_angle) / float(angular_map_dim)

    CMAP = plt.get_cmap('gnuplot')

    for i in range(angular_map_dim):
        angle_start = (angular_map_min_angle + i * angular_resolution) * 180 / np.pi + 90
        angle_end = (angular_map_min_angle + (i + 1) * angular_resolution) * 180 / np.pi + 90

        distance_cone = plt.matplotlib.patches.Wedge(
            (0.0, 0.0),
            local_maps_angular[frame][i] *
...
def render_og(frame, states, obstacle_vertices, use_grid_map, local_maps, submap_size_m,
              map_resolution, robot_radius):
    if not use_grid_map:
...
def render_video(states, last_circle_radius, robot_radius, humans_to_render, bicycles_to_render,
    ...
    def plot_agents_goals(ax, agents, color):
        for i, agent in enumerate(agents):
            agents_goal = lines.Line2D([agent.gx], [agent.gy], color=color, marker='*',
                linestyle='None', markersize=30)
            number = plt.text(agent.gx - X_OFFSET, agent.gy - Y_OFFSET,
                str(i),
                color='black',
                fontsize=22
            )
    ...
    # children
    children_positions = [[state[CHILD_STATE_INDEX][j].position for j in range(
        len(children_to_render))] for state in states]
    children = [
        plt.Circle(
    ...
    if use_grid_map:
        ...
        ax2.imshow(img_rotated, cmap="gray", vmin=0, vmax=1)
        middle_idx = int(
            round(submap_size_m / 2.0 / map_resolution))
        robot2 = plt.Circle((middle_idx, middle_idx), int(
           round(robot_radius / map_resolution)), fill=True, color='red')
        ax2.add_patch(robot2)
        ...
        ax2.set_ylim(-7, 7)
        angular_resolution = (
            angular_map_max_angle - angular_map_min_angle) / float(angular_map_dim)

        CMAP = plt.get_cmap('gnuplot')

        for ii in range(angular_map_dim):
            angle_start = (angular_map_min_angle +
                            ii * angular_resolution) * 180 / np.pi + 90
            angle_end = (angular_map_min_angle + (ii + 1)
    ...
    def update_env(frame_num):
        ...
        arrows = [patches.FancyArrowPatch(
            *orientation[frame_num], color=ARROW_COLOR,
            arrowstyle=ARROW_STYLE) for orientation in orientations]
        for arrow in arrows:
            ax.add_artist(arrow)
    ...
    def update_static_map(frame_num):
        ...
        if use_grid_map:
            img = Image.fromarray(
                local_maps[frame_num].astype('uint8'))
            img_rotated = img.rotate(90)
            ax2.imshow(img_rotated, CMAP="gray", vmin=0, vmax=1)
            middle_idx = int(
                round(
            ...
            for ii in range(angular_map_dim):
                angle_start = (
                    angular_map_min_angle + ii * angular_resolution) * 180 / np.pi + 90
                angle_end = (angular_map_min_angle + (ii + 1)
                                * angular_resolution) * 180 / np.pi + 90
                distance_cone = plt.matplotlib.patches.Wedge(
                    (0.0, 0.0),
    ...
    anim = animation.FuncAnimation(
        fig, update_env, frames=len(
            states), interval=time_step * 1000)
    anim.running = True
