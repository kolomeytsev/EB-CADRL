...
class Reward:
    ...
    def compute(self, dmin_adult, dmin_bicycle, dmin_child, collision_adult, collision_bicycle,
        ...
        if global_time >= self.time_limit:
            ...
            info = Timeout(dist_to_goal, dmin_adult, dmin_bicycle, dmin_child)
        elif collision_child:
            reward += self.collision_penalty_child
            done = True
            info = CollisionChild(dist_to_goal, dmin_adult, dmin_bicycle, dmin_child)
        elif collision_bicycle:
            reward += self.collision_penalty_bicycle
            done = True
            info = CollisionBicycle(dist_to_goal, dmin_adult, dmin_bicycle, dmin_child)
        elif collision_adult:
            reward += self.collision_penalty_adult
            done = True
            info = CollisionAdult(dist_to_goal, dmin_adult, dmin_bicycle, dmin_child)
        elif collision_obstacle:
            reward += self.collision_penalty_obstacle
            done = True
            info = CollisionObstacle(dist_to_goal, dmin_adult, dmin_bicycle, dmin_child)
